document.addEventListener("DOMContentLoaded", function() {
   ...
   function updateNavbar() {
              ...
              <ul class="nav navbar-nav ml-auto mr-0">
                  <li class="nav-item">
                      <a href="login.html"
                         ...
                         data-placement="bottom"
                         data-boundary="window">
                         <h3 style="color: white;" id="displayName"></h3>
                         </a>
                  </li>
                  <li class="nav-item">
                  <button class="btn btn-outline-white" id="logout">LOG OUT</button>
                  </li>
              </ul>
          </div>
       ...
       document.getElementById('displayName').textContent = sessionData.UserName;
       console.log("displayName1:", sessionData.UserName);
   }

   // Call updateNavbar to initially update the navbar
   updateNavbar();

   // Logout button event listener
   document.getElementById('logout').addEventListener('click', function() {
       // Use fetch to send a POST request to the logout.php script in the api folder
       fetch('api/logout.php', {
           method: 'POST',
       })
       .then(response => {
           if (response.ok) {
               // Clear username from localStorage upon logout
